...
bool cylinder(Coord bottom, Coord top, double r, int n, Vector** vec) {
    ...
    for( int i = 0; i < n; i++ ) {
        vec[i] = new Vector();
        vec[i]->setBegin( x_b + r*cos( (double)i*angle/rad ), y_b, z_b + r*sin( (double)i*angle/rad ) );
        vec[i]->setEnd( x_b + r*cos( (double)(i+1)*angle/rad ), y_b, z_b + r*sin( (double)(i+1)*angle/rad ) );
    }
    
    //wektory do rysowania górnej podstawy
    for( int i = n, j = 0; i < 2*n; i++, j++ ) {
        vec[i] = new Vector();
        vec[i]->setBegin( x_t + r*cos( (double)j*angle/rad ), y_t, z_t + r*sin( (double)j*angle/rad ) ); 
        vec[i]->setEnd( x_t + r*cos( (double)(j+1)*angle/rad ), y_t, z_t + r*sin( (double)(j+1)*angle/rad ) );
    }
    /////////////////////////////
...
int main() {
	...
	cylinder( Coord(1,0,-2), Coord(-1,1,2), r, n, vec );
	
	FILE* fp;
	fp = fopen("cylinder.geo", "w");
	while(k < 3*n) {
		cout << vec[k]->toString() << endl;
		fprintf( fp, "%s", vec[k]->toFile().c_str() );
        ++k;
	}
	fclose(fp);
